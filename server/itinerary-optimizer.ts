...
export async function generateOptimizedItineraries(
  ...
  try {
    // Fetch anchor and boundary constraints if tripId provided
...
Respond with valid JSON in this exact format:
{
  "variants": [
    ...
    for (let v = 0; v < aiResponse.variants.length; v++) {
      ...
      // Apply anchor constraints: pin anchored items and reschedule around them
      if (anchorConstraints.length > 0 || boundaryConstraints.length > 0) {
        ...
        for (const [dayStr, dayItems] of Object.entries(dayGroups)) {
          ...
          const { activities: adjusted, anchorNotes } = applyAnchorConstraints(dayItems, dayAnchors, dayBoundary);
          anchorAdjusted.push(...adjusted);
          methodologyNotes.push(...anchorNotes);
        }

        // Apply energy balancing after anchor constraints
        const { activities: energyBalanced, energyNotes } = applyEnergyBalancing(anchorAdjusted);
        reorderedItems = energyBalanced;
        methodologyNotes.push(...energyNotes);
      }
